  ...
  <Wrapper dr={`row`}>
   ...
   <Wrapper width={`70%`}>
    ...
    <Wrapper>
     ...
     {/**Bottom */}
     {joinBannerDatum ? (
      joinBannerDatum.length === 0 ? (
       <Wrapper>인사글이 없습니다.</Wrapper>
      ) : (
       joinBannerDatum.map((data, idx) => {
        return (
         <BottomBoard
          key={idx}
          onClick={() => moveLinkHandler(`Join_D/${data._id}`)}
         >
          <Wrapper width={`5%`} padding={`5px`}>
           {/** 번호 */}
     ...
     <Wrapper ju={`flex-end`} dr={`row`}>
      {pages && pages.length > 0 && (
       <PagenationWrapper width={`auto`}>
        <PagenationBtn
         onClick={() =>
          joinBannerDatum && prevAndNextPageChangeNoticeHandler(currentPage - 1)
         }
        >
         <IoIosArrowBack />
        </PagenationBtn>
